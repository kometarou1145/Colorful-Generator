...
while cnt < 100:
 ...
 code2 = ''.join([random.choice(string.ascii_lowercase + string.digits) for i in range(33)])
 index9 = random.randint(0, len(Color) - 1)
 ColorRandom8 = Color[index9]
 with open('gigafiles.txt', 'a') as gigafiles:
  print(f"https://17.gigafile.nu/{code}-{code2}", file=gigafiles)
  print(Underline + Bold + ColorRandom8 + f"Generated : https://17.gigafile.nu/{code}-{code2}" + Reset)
cnt += 1