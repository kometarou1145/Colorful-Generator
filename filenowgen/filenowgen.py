...
cnt = 0
while cnt < 100:
 code = ''.join([random.choice(string.ascii_lowercase + string.digits) for i in range(9)])
 index9 = random.randint(0, len(Color) - 1)
 ColorRandom8 = Color[index9]
 with open('filenows.txt', 'a') as filenows:
  print(f"https://d.kuku.lu/{code}", file=filenows)
  print(Underline + Bold + ColorRandom8 + f"Generated : https://d.kuku.lu/{code}" + Reset)
cnt += 1